/**************  Effects Program  *********************/
// atr 12/5/14 - tried to add an FIR filter
...
// run the waveform through the matched filter
  for ( unsigned int i=0; i<2*frames.size(); i++ ) {
	  if (i < frames.size()) {
		fout = data.Matched_filter.tick(frames[i]);
		std::cout << frames[i] << ", " << fout << "\n";
	  }
	  else {
		fout = data.Matched_filter.tick(0);
		std::cout << 0 << ", " << fout << "\n";
	  }	  
  }  
